# experiments/runtime_breakdown_ba_sparse_rhs.py
# -*- coding: utf-8 -*-
...
from inc.diffus import b_estim, SIR_STATES
from inc.utils import seed_all
import hermes as hm


# ============================================================
# monkey patch: only patch q_train hot path (lik_ms / _lik_step)
# ============================================================
class SparseTrainQNet(hm.QNet):
    def _empty_sp(self, size):
        idx = torch.empty((2, 0), dtype=torch.long, device=self.device)
        val = torch.empty((0,), dtype=self.adj.dtype, device=self.device)
        return torch.sparse_coo_tensor(
            idx, val, size=size, dtype=self.adj.dtype, device=self.device
        ).coalesce()

    def _bool_to_sp(self, mask):
        mask = mask.bool()
        idx = mask.nonzero(as_tuple=False).T
        if idx.numel() == 0:
            return self._empty_sp(tuple(mask.shape))
        val = torch.ones(idx.size(1), dtype=self.adj.dtype, device=mask.device)
        return torch.sparse_coo_tensor(
            idx,
            val,
    ...
    def _safe_spmm(self, rhs_sp):
        """
        sparse(self.adj) @ sparse(rhs_sp)
        If sparse@sparse is not happy on the current backend, fallback safely.
        """
        try:
            out = torch.sparse.mm(self.adj, rhs_sp)
            if out.is_sparse:
                return out.coalesce()
            return out.to_sparse_coo().coalesce()
        except RuntimeError:
            out = torch.sparse.mm(self.adj, rhs_sp.to_dense())
            if out.is_sparse:
                return out.coalesce()
            return out.to_sparse_coo().coalesce()

    def _spmm_bool_dense(self, mask):
        rhs_sp = self._bool_to_sp(mask)
        out_sp = self._safe_spmm(rhs_sp)
        return out_sp.to_dense()

    def _selector_sp(self, rows, active_cols, n_samples):
        cols = active_cols.nonzero(as_tuple=False).flatten()
        if cols.numel() == 0:
            return self._empty_sp((self.n_nodes, n_samples))

        if torch.is_tensor(rows):
            rows = rows.to(device=self.device, dtype=torch.long)
            if rows.dim() == 0:
                rows = rows.expand(cols.numel())
            else:
                rows = rows[cols]
        else:
            rows = torch.full(
        ...
        val = torch.ones(cols.numel(), dtype=self.adj.dtype, device=self.device)
        return torch.sparse_coo_tensor(
            idx,
            val,
            size=(self.n_nodes, n_samples),
            dtype=self.adj.dtype,
            device=self.device,
        ).coalesce()

    def _neighbor_delta_dense(self, rows, active_cols, n_samples):
        rhs_sp = self._selector_sp(rows, active_cols, n_samples)
        if rhs_sp._nnz() == 0:
            return torch.zeros(
        ...
        out_sp = self._safe_spmm(rhs_sp)
        return (out_sp.to_dense() > 0).to(torch.long)

    def _lik_step(self, y0, y1, lI1, lI0, lR1, lR0, yL=None, reach=None):
        # y*, l*, reach: (nodes, samples)
        ...
        # I -> S
        uid = lI1.argsort(dim=0, descending=True)
        msk = (y1 == SIR_STATES.I) | (msk & (y0 != SIR_STATES.R))

        rem = torch.where(
            msk,
            (reach.long() if reach is not None else 1) + self._spmm_bool_dense(msk).long(),
            self.n_inf,
        )
        ...
        for i, u in enumerate(uid):
            ...
            rem_v = rem_v.gather(dim=0, index=u.unsqueeze(dim=0)).squeeze(dim=0)
            rem_u = rem.gather(dim=0, index=u.unsqueeze(dim=0)).squeeze(dim=0)

            opt = (rem_u > 1) & (rem_v > 1)
            if yL is not None:
                opt = opt & (yL.gather(dim=0, index=u.unsqueeze(dim=0)).squeeze(dim=0) != SIR_STATES.I)

            msk_u = msk.gather(dim=0, index=u.unsqueeze(dim=0)).squeeze(dim=0)
            msk_opt = msk_u & opt

            # keep original semantics
            lik = lik + torch.where(msk_opt, torch.where(y0 == SIR_STATES.S, lI1, lI0), self.zero)

            trs = (y0.gather(dim=0, index=u.unsqueeze(dim=0)).squeeze(dim=0) == SIR_STATES.S)
            delta = self._neighbor_delta_dense(u, (msk_u & trs), n_samples).to(rem.dtype)
            rem = rem - delta
    ...
    def lik_ms(self, Y, obs_time):
        ...
        zI0, zR0, zI, zR = self.forward(y_cond, orig=True)
        ...
        lI1, lI0 = torch.nn.functional.logsigmoid(zI), torch.nn.functional.logsigmoid(-zI)
        lR1, lR0 = torch.nn.functional.logsigmoid(zR), torch.nn.functional.logsigmoid(-zR)
        ...
        for i in range(K):
            ...
            if TL is None:
                ...
                yL_not_R = (yL != SIR_STATES.R)
                for d in range(1, L):
                    nbr = self._spmm_bool_dense(reach[d - 1]) > 0
                    reach[d] = reach[d - 1] | (nbr & yL_not_R)
                for t in range(TR - 1, TL, -1):
                    lik = lik + self._lik_step(
                        Y[t], Y[t + 1],
                        lI1[t], lI0[t], lR1[t], lR0[t],
                        yL=yL,
                        reach=reach[t - TL],
                    )
...
def compose_history(data, tI, tR):
    ...
    y_pred.scatter_(
        ...
        src=torch.full_like(tI, SIR_STATES.I),
    )
    y_pred.scatter_(
        dim=1,
        index=torch.minimum(tR, data.T),
        src=torch.full_like(tR, SIR_STATES.R),
    )
...
def run_one_size(requested_graph_size, run_order, args):
    ...
    row = init_row(requested_graph_size, run_order, obs_time)
    ...
    try:
        ...
        row["total_wall_sec"] = float(row["graph_build_sec"] + row["total_algo_sec"])
        row["status"] = "ok"
...
def make_plot(rows, fig_path, q_steps):
    ...
    for i, total in enumerate(totals):
        ax.text(i, total, f"{total:.1f}s", ha="center", va="bottom", fontsize=9)

    ax.set_xticks(x)
    ax.set_xticklabels(xticklabels)
    ax.set_xlabel("BA graph size n")
    ax.set_ylabel("running time (sec)")
    ax.set_title(f"HERMES runtime breakdown on BA-SIR (sparse RHS, q_steps={q_steps})")
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Runtime breakdown on BA-SIR with sparse RHS patch for proposal training.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument("--graph_sizes", type=str, default="50000,30000,1000")
    parser.add_argument("--seed", type=int, default=123456789)
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--output_dir", type=str, default="output/runtime_breakdown_ba_sparse_rhs")

    # BA-SIR generation
    parser.add_argument("--T", type=int, default=10)
    parser.add_argument("--obs_time", type=str, default="5")
    parser.add_argument("--ba_m", type=int, default=4)
    parser.add_argument("--fraction_infected", type=float, default=0.05)
    parser.add_argument("--sim_pI", type=float, default=0.1)
    parser.add_argument("--sim_pR", type=float, default=0.1)
    ...
    # proposal training
    parser.add_argument("--q_steps", type=int, default=200)
...
def main():
    ...
    csv_path = os.path.join(args.output_dir, "runtime_breakdown_ba_sparse_rhs.csv")
    fig_path = os.path.join(args.output_dir, "runtime_breakdown_ba_sparse_rhs.png")

    print("=" * 80, flush=True)
    print("BA-SIR runtime breakdown experiment (sparse RHS patch)", flush=True)
    ...
    for run_order, requested_graph_size in enumerate(graph_sizes, start=1):
        print(f"\n=== [{run_order}/{total_runs}] running BA-SIR with n={requested_graph_size} ===", flush=True)
        row = run_one_size(
